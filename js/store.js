// js/store.js
// منطق صفحات المتجر بالكامل ومصلح بالكامل لمعالجة خطأ تنزيل الكيلوات
...
function productCardHTML(product, context) {
  ...
  // فحص ذكي للوحدة لتحديد نقطة البداية والأزرار
  const isWhole = (product.unit !== 'كيلو' && product.unit !== 'لتر');
  const minQty = isWhole ? 1 : 0.25;

  if (!StoreState.qtyMap[uid] || StoreState.qtyMap[uid] < minQty) {
    StoreState.qtyMap[uid] = minQty;
  }
    ...
    <div class="product-card">
      ...
      <div class="qty-row">
        <button class="qty-btn" onclick="changeQty('${uid}', ${product.id}, -1)"
          ${qty <= minQty ? 'disabled' : ''}>−</button>
        <span class="qty-val" id="qty-${uid}">${isWhole ? qty.toFixed(0) : qty.toFixed(2)}</span>
        <button class="qty-btn" onclick="changeQty('${uid}', ${product.id}, 1)"
          ${qty >= 100 ? 'disabled' : ''}>+</button>
      </div>
...
function changeQty(uid, productId, dir) {
  ...
  const isWhole = (p.unit !== 'كيلو' && p.unit !== 'لتر');
  const STEP = isWhole ? 1 : 0.25;
  const minQty = isWhole ? 1 : 0.25;

  let qty = StoreState.qtyMap[uid] || minQty;
  qty = dir > 0
    ? Math.min(100, parseFloat((qty + STEP).toFixed(2)))
    : Math.max(minQty, parseFloat((qty - STEP).toFixed(2)));
  ...
  if (qtyEl) qtyEl.textContent = isWhole ? qty.toFixed(0) : qty.toFixed(2);
  if (totalEl) totalEl.textContent = (p.price * qty).toFixed(2) + ' جنيه';
  ...
  if (card) {
    ...
    if (plusBtn)  plusBtn.disabled  = qty >= 100;
  }
}
    ...
    return `
      <div class="cart-item">
        ...
        <div class="cart-item-info">
          ...
          <p class="cart-item-sub">${p.price} جنيه × ${isWhole ? item.qty.toFixed(0) : item.qty.toFixed(2)} ${p.unit}</p>
        </div>
        <div class="qty-row">
          <button class="qty-btn"
            onclick="cartChangeQty(${p.id}, ${parseFloat((item.qty - STEP).toFixed(2))})">−</button>
          <span class="qty-val">${isWhole ? item.qty.toFixed(0) : item.qty.toFixed(2)}</span>
          <button class="qty-btn"
            onclick="cartChangeQty(${p.id}, ${parseFloat((item.qty + STEP).toFixed(2))})">+</button>
        </div>
      ...
      ${Cart.items.map(item => {
        ...
        return `
          <div class="checkout-item">
            <span>${p.name} × ${isWhole ? item.qty.toFixed(0) : item.qty.toFixed(2)} ${p.unit}</span>
